...
def main():
    ...
    for date in range(0, totalday):
        ...
        for runhour in range(0,duration):
            ...
            [ponsets_final, sonsets_final] = picking.pick(points, outputdir+day+str(int(float(time_begin['hour'])+runh)%24)+'_',nametag, nsta, order, pp, ptraveltimes, straveltimes, step, win, kurwindow, sr, dstop, vfilter, h1filter, h2filter, vfilter2, h1filter2, h2filter2, savefigure, overlap)
